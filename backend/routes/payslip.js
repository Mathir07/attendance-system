...
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    const dir = path.join(__dirname, '../uploads/payslips', String(req.params.userId || req.body.user_id || 'temp'))
...
router.post('/upload/:userId', auth, requireHR, upload.single('payslip'), async (req, res) => {
  try {
    ...
    const employee = await db.getAsync('SELECT id,name FROM users WHERE id = ?', [req.params.userId])
    if (!employee) return res.status(404).json({ message: 'Employee not found' })

    const relativePath = path.join('payslips', req.params.userId, req.file.filename)

    await db.runAsync(
      `INSERT INTO payslips (user_id, month, year, file_path, net_pay, uploaded_by)
       VALUES (?,?,?,?,?,?)
       ON CONFLICT(user_id,month,year) DO UPDATE SET
         file_path=excluded.file_path, net_pay=excluded.net_pay,
         generated_at=datetime('now'), uploaded_by=excluded.uploaded_by`,
      [req.params.userId, month, year, relativePath, net_pay || null, req.user.id]
    )

    await db.runAsync(
      `INSERT INTO audit_logs (actor_id,actor_name,action,target_table,details)
       VALUES (?,?,?,?,?)`,
      [req.user.id, req.user.name, 'UPLOADED_PAYSLIP', 'payslips',
...
router.get('/my', auth, async (req, res) => {
  try {
    const rows = await db.allAsync(
      `SELECT id, month, year, net_pay, generated_at FROM payslips
...
router.get('/download/:id', auth, async (req, res) => {
  try {
    const slip = await db.getAsync('SELECT * FROM payslips WHERE id = ?', [req.params.id])
    ...
    const filePath = path.join(__dirname, '../uploads', slip.file_path)
    if (!fs.existsSync(filePath))
      return res.status(404).json({ message: 'File not found on server' })

    res.download(filePath, `payslip-${slip.year}-${String(slip.month).padStart(2,'0')}.pdf`)
  } catch (err) {
    res.status(500).json({ message: 'Server error', error: err.message })
  }
})
